...
// ── Overview ────────────────────────────────────────────────────────────────
export function overviewPage(d: Data, readmeHtml: string | null, config: ConfigGraph): string {
  ...
  const readmeBlock = readmeHtml
    ...
    : `<section class="note">No <code>README.md</code> in this package. This overview is synthesized from <code>package.json</code> and the composition root (<code>src/bootstrap.ts</code>).</section>`;
  ...
  const stats: { n: number; l: string }[] = [
    ...
    { n: count((s) => !s.container && s.visibility === "public-api"), l: "app surface" },
  ];
...
<div class="cols">
  <div class="col-main">
    ...
    <div class="mod-grid">${cards}${configSummaryCard(config, l)}</div>
    ${readmeBlock}
    <h2>App surface <span class="dim">wired from bootstrap.ts · ${String([...publicByModule.values()].reduce((n, a) => n + a.length, 0))}</span></h2>

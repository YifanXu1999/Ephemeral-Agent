...
function main(): void {
  ...
  // Configuration tab
  for (const cp of [
    configIndexPage(config),
    configAgentsPage(config),
    configWorkflowsPage(config),
    configHooksPage(config),
    configClientsPage(config),
  ]) {
    write(cp.path, cp.html);
  }
  ...
  const pages = 1 + d.modules.length + 2 + 5;
  process.stdout.write(`[render] ${String(pages)} pages (+5 config) + 5 assets → code_map/ (entry: code_map_index.html)\n`);
}

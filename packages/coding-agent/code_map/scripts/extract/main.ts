...
import { buildConfigGraph, parseConfigs } from "./config/configs.js";
...
function main(): void {
  ...
  const parsedConfigs = parseConfigs(PKG_ROOT);
  const configGraph = buildConfigGraph(PKG_ROOT, parsedConfigs, symbols);
  process.stdout.write(
    `[extract] config: ${String(configGraph.agents.length)} agents, ${String(configGraph.workflows.length)} workflows, ${String(configGraph.hooks.length)} hooks, ${String(configGraph.llmClients.length)} clients\n`,
  );
  ...
  const manifest: Manifest = {
    ...
    counts: {
      ...
      workflows: configGraph.workflows.length,
      llmClients: configGraph.llmClients.length,
    },
  };
  ...
  writeJson("config-graph.json", configGraph);

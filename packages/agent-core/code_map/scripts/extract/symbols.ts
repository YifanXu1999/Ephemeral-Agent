...
/** Walk src/index.ts exports to the original declarations + type-only flags. */
function publicSurface(ctx: Ctx): {
  ...
  const index = ctx.program
    .getSourceFiles()
    .find((sf) => ctx.rel(sf.fileName) === "src/index.ts");
  if (!index) return { decls, typeOnly };

  for (const st of index.statements) {
    if (ts.isExportDeclaration(st) && st.exportClause && ts.isNamedExports(st.exportClause)) {
      const blockTypeOnly = st.isTypeOnly;
      for (const spec of st.exportClause.elements) {
        typeOnly.set(spec.name.text, blockTypeOnly || spec.isTypeOnly);
      }
    }
  }

  const idxSym = ctx.checker.getSymbolAtLocation(index);
  if (idxSym) {
    for (const exp of ctx.checker.getExportsOfModule(idxSym)) {
      let s: ts.Symbol = exp;

...
<title>${esc(o.title)} · @ephai/agent-core</title>
<link rel="stylesheet" href="${l.css}">
</head><body class="${o.bodyClass ?? ""}">
<header class="topbar">
  <a class="brand" href="${l.index}">@ephai/agent-core<span>explorer</span></a>
  <input id="q" class="search" type="search" placeholder="Search symbols, files…  ( / )" autocomplete="off" data-shell="${l.shell}">
  <nav class="tabs">${tab("overview", "Overview", l.index)}${tab("code", "Code Map", l.shell)}${tab("coverage", "Coverage", l.coverage)}</nav>
</header>
